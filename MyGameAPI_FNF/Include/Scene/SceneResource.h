...
public:
	...
	void SetTextureColorKey(const std::string& Name,
		const unsigned char r, const unsigned char g, const unsigned char b, int Index = 0);
...
public:
	...
	void AddAnimationFrameData(const std::string& SequenceName, const Vector2& StartPos, const Vector2& Size, const Vector2& Frame = Vector2(0.f, 0.f));
	void AddAnimationFrameData(const std::string& SequenceName, float PosX, float PosY, float SizeX, float SizeY, float FrameX = 0.f, float FrameY = 0.f);
...
public:
	...
	bool LoadMusic(const std::string& GroupName, bool Loop, const std::string& Name, const char* FileName, const std::string& PathName = MUSIC_PATH);
...
public:
	bool LoadSongAllData();
	
	SongInfo* FindSongData(const std::string& Name);
};
