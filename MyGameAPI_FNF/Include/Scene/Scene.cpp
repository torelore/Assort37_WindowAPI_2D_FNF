...
CScene::CScene()
{
	...
	m_NoteCount = 0;
	m_NoteCapacity = 100;
	m_NoteArray = new CGameObject * [m_NoteCapacity];
...
bool CScene::Update(float DeltaTime)
{
	...
	{
		auto	iter = m_NoteList.begin();
		auto	iterEnd = m_NoteList.end();

		for (; iter != iterEnd;)
		{
			if (!(*iter)->IsActive())
			{
				iter = m_NoteList.erase(iter);
				iterEnd = m_NoteList.end();
				continue;
			}
...
bool CScene::PostUpdate(float DeltaTime)
{
	...
	{
		std::list<CSharedPtr<CGameObject>>::iterator	iter = m_NoteList.begin();
		std::list<CSharedPtr<CGameObject>>::iterator	iterEnd = m_NoteList.end();
...
bool CScene::Render(HDC hDC)
{
	...
	// Note Render
	{
		...
		for (; iter != iterEnd;)
		{
			...
			if (!(*iter)->IsCull())
			{
				if (m_NoteCount == m_NoteCapacity)
				{
					m_NoteCapacity *= 2;

					CGameObject** Array = new CGameObject * [m_NoteCapacity];

					memcpy(Array, m_NoteArray, sizeof(CGameObject*) * m_NoteCount);
	...
	// 출력 목록을 정렬한다.
	qsort(m_NoteArray, (size_t)m_NoteCount, sizeof(CGameObject*), CScene::SortY);

	for (int i = 0; i < m_NoteCount; ++i)
	{
		m_NoteArray[i]->Render(hDC);
	}
