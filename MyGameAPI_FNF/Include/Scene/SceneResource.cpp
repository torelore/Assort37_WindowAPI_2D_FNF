...
void CSceneResource::AddAnimationFrameData(const std::string& SequenceName, const Vector2& StartPos, const Vector2& Size, const Vector2& Frame)
{
...
void CSceneResource::AddAnimationFrameData(const std::string& SequenceName, float PosX, float PosY, float SizeX, float SizeY, float FrameX, float FrameY)
{
	CAnimationSequence* Sequence = FindAnimationSequence(SequenceName);
...
bool CSceneResource::LoadMusic(const std::string& GroupName, bool Loop, const std::string& Name, const char* FileName, const std::string& PathName)
{
	if (FindSound(Name))
		return true;
...
bool CSceneResource::LoadSongAllData()
{
	if (!CResourceManager::GetInst()->LoadAllSongData()) {
		return false;
	}
...
SongInfo* CSceneResource::FindSongData(const std::string& Name)
{
	SongInfo* Info = CResourceManager::GetInst()->FindSongInfo(Name);
