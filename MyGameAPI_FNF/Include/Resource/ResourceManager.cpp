...
bool CResourceManager::LoadMusic(const std::string& GroupName, bool Loop, const std::string& Name, const char* FileName, const std::string& PathName)
{
	CSound* Sound = FindSound(Name);
...
bool CResourceManager::LoadAllSongData()
{
	CSongData* SongData = new CSongData;

	if (!SongData->LoadAllSongData()) {
		SAFE_DELETE(SongData);

		return false;
	}

	m_mapSongData = SongData->GetMapSongData();

	return true;
}

std::vector<std::string> CResourceManager::GetAllSongName() const
{
	std::vector<std::string> vecSongName;

	vecSongName.push_back("Tutorial");

	auto iter = m_mapSongData.begin();
	auto iterEnd = m_mapSongData.end();

	for (; iter != iterEnd; iter++) {
		if (iter->first == "Tutorial")
			continue;

		vecSongName.push_back(iter->first);
	}

	return vecSongName;
}

std::string CResourceManager::GetStoryModeSongName() const
{
	std::string SongName = "";
...
SongInfo* CResourceManager::FindSongInfo(const std::string& Name)
{
	auto	iter = m_mapSongData.find(Name);

	if (iter == m_mapSongData.end())
		return nullptr;
