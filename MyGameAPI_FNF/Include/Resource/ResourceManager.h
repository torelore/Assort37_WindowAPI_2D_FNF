...
private:
	...
	std::unordered_map<std::string, SongInfo*>	m_mapSongData;
...
public:
	...
	std::vector<std::string> GetAllSongName() const;

	SongInfo* FindSongInfo(const std::string& Name);

public :
	std::string	GetStoryModeSongName() const;
	
	void SetStoryModeNext() {
		m_StoryModeNum++;
	}
